...
void Program::displayVisDijkstraScreenGUI() {
	...
	if (ImGui::Button("Skip to Start")) {
		visEngine_Dijkstra.skipToStartState();
	}
	// ImGui::EndDisabled();
	ImGui::SameLine();
	ImGui::BeginDisabled(!visEngine_Dijkstra.animPaused);
	...
	if (ImGui::BeginTabBar("DijkstraTabBar")) {
	...
	// -- EXPORT DATA STRUCTURE TO STRING --
	if (ImGui::BeginTabItem("Export")) {
		static GraphReader::GraphInitMethod graphDijkstraExportOption = GraphReader::GraphInitMethod::ADJ_MATRIX; // State variable to hold the selected option's value
		ImGui::RadioButton("Adjacency Matrix", (int*)&graphDijkstraExportOption, (int)GraphReader::GraphInitMethod::ADJ_MATRIX); // Value 0
		ImGui::SameLine();
		ImGui::RadioButton("Adjacency List", (int*)&graphDijkstraExportOption, (int)GraphReader::GraphInitMethod::ADJ_LIST); // Value 1
		ImGui::SameLine();
		ImGui::RadioButton("Edge List", (int*)&graphDijkstraExportOption, (int)GraphReader::GraphInitMethod::EDGE_LIST); // Value 2

		std::string exportGraphString;
		switch (graphDijkstraExportOption) {
		case GraphReader::GraphInitMethod::ADJ_MATRIX:
			exportGraphString = visEngine_Dijkstra.graph.printAdjMatString();
			break;
		case GraphReader::GraphInitMethod::ADJ_LIST:
			exportGraphString = visEngine_Dijkstra.graph.printAdjListString();
			break;
		case GraphReader::GraphInitMethod::EDGE_LIST:
			exportGraphString = visEngine_Dijkstra.graph.printEdgeListString();
			break;
		}

		if (ImGui::Button("Copy to Clipboard")) {
			SetStringToClipboard(exportGraphString);
		}
		ImGui::InputTextMultiline("##readonlyGRAPH_EXPORT", exportGraphString.data(), exportGraphString.size() + 1U,
